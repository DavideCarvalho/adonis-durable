---
'@agora/durable-cli': minor
---

Add `@agora/durable-cli`: AdonisJS ace commands to operate the durable workflow engine.
...
resolve the `WorkflowEngine` bound by `@agora/durable`'s provider; `node ace configure` registers the
