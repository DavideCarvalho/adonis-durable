...
description: Catch Date.now(), Math.random(), new Date() and crypto.randomUUID() inside a workflow body at author time with @adonis-agora/durable-eslint-plugin — an AST-scoped ESLint rule that knows both the engine.register() function form and a @Workflow-decorated class.
---
...
- **The class form** — the `run` method of a class decorated with `@Workflow` (whether written `@Workflow` or `@Workflow({ ... })`).
