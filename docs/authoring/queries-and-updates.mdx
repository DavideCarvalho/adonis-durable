...
engine.register('video-encode', '1', async (ctx, job: EncodeJob) => {
  ...
  const segments = await ctx.step('probe', () => encoder.probe(job.src))
...
export default class JobsController {
  ...
  async progress({ params }: HttpContext) {
    ...
    return progress ?? { phase: 'pending', pct: 0 }
  }
}
```
...
engine.register('expense-approval', '1', async (ctx, expense: Expense) => {
  ...
  await ctx.step('reimburse', () => ledger.reimburse(expense, decision.approver))
