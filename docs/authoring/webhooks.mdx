...
Configure the builder once, on the engine deps (via `config/durable.ts` or the dependency object you pass `WorkflowEngine`):
...
```ts
import { remoteStep, FatalError } from '@adonis-agora/durable'
...
engine.register('checkout', '1', async (ctx, order: Order) => {
  ...
  await ctx.step('fulfill', () => fulfil(order, result.providerRef))
...
```ts
// start/routes.ts
router.post('/durable/webhooks/:token', async ({ params, request }) => {
