...
Alongside the trace, the **full Agora request context** (userRef / tenant / traceId) rides each remote task automatically. When [`@adonis-agora/context`](/docs/context) is installed, the `@adonis-agora/durable` provider snapshots the live context onto the dispatched `RemoteTask` (read structurally from the global accessor slot, no hard dependency); before a worker runs the step handler, durable restores that snapshot into the worker's context. So a dispatched `ctx.step(ref, input)` on a worker sees the originating request's userRef / tenant / traceId with **zero manual serialize/deserialize** — best-effort, and a no-op when `@adonis-agora/context` is not installed.
