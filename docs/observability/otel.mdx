...
The `@agora/durable/otel` subpath turns workflow runs into OpenTelemetry traces — a root span per run, a child span per step — so they show up in the tracing stack you already run. It ships in the main `@agora/durable` package; install `@opentelemetry/api` as the only extra peer.
...
import { attachDurableOtel } from '@agora/durable/otel'
...
```ts
import { attachDurableMetrics } from '@agora/durable/otel'
...
Core stays OTel-free, so you wire the provider in. Use `otelTraceparent` from `@agora/durable/otel`, which reads the current active span via the globally-registered W3C propagator:

```ts title="config/durable.ts"
import { defineConfig } from '@agora/durable'
import { otelTraceparent } from '@agora/durable/otel'
...
> When [`@agora/diagnostics-otel`](/docs/diagnostics) is installed, the `@agora/durable` provider auto-wires this `traceparent` provider for you (read structurally from a global slot, no hard dependency) — so distributed tracing works with zero config in that setup.
