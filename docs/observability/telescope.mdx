...
Already running [Telescope](/docs/telescope)? The `@agora/durable/telescope` subpath adds a dedicated **Workflows** view so durable flows appear next to your requests, queries, and jobs — with a health dashboard built from the engine's lifecycle events and live store reads. It ships in the main `@agora/durable` package — no extra install.
