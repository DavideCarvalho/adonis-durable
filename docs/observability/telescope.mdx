...
Already running [Telescope](/docs/telescope)? `@agora/durable-telescope` adds a dedicated **Workflows** view so durable flows appear next to your requests, queries, and jobs — with a health dashboard built from the engine's lifecycle events and live store reads.

```bash
npm i @agora/durable-telescope
```
