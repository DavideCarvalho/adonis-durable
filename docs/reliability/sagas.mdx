...
A durable run often performs several irreversible side effects in sequence — reserve inventory, charge a card, allocate a shipment. If a later step fails, the earlier effects are still out there in the world, and "the run failed" is not an acceptable end state when money has moved. The saga pattern handles this: alongside each step that *does* something, you register how to *undo* it, and when the run fails the engine runs those undos in reverse order. `@agora/durable` builds this in via a `compensate` option on `ctx.step`.
...
engine.register('checkout', '1', async (ctx, order: Order) => {
  ...
  const label = await ctx.step('allocate-shipment', () => shipping.allocate(order, reservation.id))
