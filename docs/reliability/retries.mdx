...
A remote step declared with `remoteStep` and invoked with `ctx.call` runs on a worker — possibly in another process. There are **two distinct paths** for handling its failures, chosen by whether the step sets `timeoutMs`.
...
Setting `timeoutMs` on a remote step opts it into a different path. `timeoutMs` is a **liveness window**: if the worker produces neither a result nor a heartbeat within that many ms, the engine presumes it dead, fails the dispatch with a `RemoteStepTimeout`, and — because that timeout is retryable — re-dispatches it up to `retries`. Each heartbeat the worker emits (via `transport.onHeartbeat`) rearms the window, so a long but healthy step that keeps beating stays alive well past `timeoutMs`.
