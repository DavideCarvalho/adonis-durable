...
The `@agora/durable/admission-redis` subpath replaces the per-instance admission accounting with a Redis-backed one, so a `concurrency` or `rateLimit` cap holds **across every engine instance**. It's the tier you reach for when several worker pods must share one global limit. It ships in the main `@agora/durable` package; install `@adonisjs/redis` (or `ioredis`) as the extra peer.
