...
7. **The `@Workflow` decorator is removed** — author workflows with `BaseWorkflow` + `static workflow = { name, version }`. Replace `@Workflow({ name, version }) class X` with `class X extends BaseWorkflow { static workflow = { name, version } }`.
