---
title: SQL (database)
description: A broker-less, DBOS-style transport driver — remote steps are rows in the Lucid database you already run. Workers claim tasks with an atomic, portable lease so a row is never run twice. The migration ships with @agora/durable.
---

import { Callout } from 'fumadocs-ui/components/callout';

The `db` driver needs **no broker**. A remote step is a **row** in the same database your durable store already uses: `ctx.call` inserts a task row, a worker claims it with an atomic lease, runs the handler, writes a result row, and the engine polls that row to resume the run. The database you already operate *is* the queue.

It ships **inside `@agora/durable`** — `@adonisjs/lucid` is an optional peer, imported lazily only when you select the `db` transport. The transport tables come from a migration published when you configure the package:
...
A worker process constructs the transport pinned to the group it serves and registers handlers:

```ts title="start/worker.ts (a separate worker process)"
import db from '@adonisjs/lucid/services/db'
import { DbTransport } from '@agora/durable'

const transport = new DbTransport({ db, group: 'pipeline' })
