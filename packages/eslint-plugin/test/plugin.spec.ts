...
describe('plugin', () => {
  ...
  it('exposes a flat-config recommended preset that turns the rule on', () => {
    ...
    expect(recommended.plugins['@agora/durable']).toBe(plugin);
    expect(recommended.rules['@agora/durable/no-nondeterminism']).toBe('error');
  });

  it('carries plugin meta', () => {
    expect(plugin.meta.name).toBe('@agora/durable-eslint-plugin');
  });
});
