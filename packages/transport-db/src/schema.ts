...
/**
 * The transport's table names. Distinct from the durable *store* tables (`durable_workflow_runs`
 ...
 * (`node ace configure @agora/durable-transport-db`); this helper is for standalone use and tests.
 */
export async function createDurableTransportTables(db: Database): Promise<void> {
  ...
  // ── control: best-effort control-plane messages (single-consumer; see DESIGN.md) ──
