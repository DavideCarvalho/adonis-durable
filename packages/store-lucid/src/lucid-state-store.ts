...
import { DURABLE_TABLES, createDurableTables } from './schema.js';
 ...
 * Wire it via `config/durable.ts`: `store: lucidStateStore(db)`.
 */
...
/** Factory mirroring the other Agora adapters' `xStore(db)` ergonomics for `config/durable.ts`. */
export function lucidStateStore(db: Database, options?: LucidStateStoreOptions): LucidStateStore {
  return new LucidStateStore(db, options);
}
