...
/**
 * Dependency-free, framework-light signed session cookie for the dashboard's
 * built-in {@link file://./auth.ts dashboardAuth} mechanism. Stateless
 * HMAC-SHA256 over `node:crypto` only (no JWT dependency, no store, no
 * AdonisJS types) — ported verbatim from `@dudousxd/nestjs-durable` so the
 * cookie format is identical across the two dashboards and this stays
 * unit-testable without an `HttpContext`.
 */
 ...
 */
export function signSessionCookie(user: DashboardSessionUser, options: SignOptions): string {
